...
conf_matrix_tree <- confusionMatrix(predictions_tree, test_data$num_interest)

roc_curve_tree <- roc(test_data$num_interest, as.numeric(predictions_tree))
auc_value_tree <- auc(roc_curve_tree)

cat("Decision Tree Confusion Matrix:\n", conf_matrix_tree$table, "\n")
cat("Decision Tree Accuracy:", conf_matrix_tree$overall["Accuracy"], "\n")
cat("Decision Tree Precision:", conf_matrix_tree$byClass["Pos Pred Value"], "\n")
cat("Decision Tree Recall (Sensitivity):", conf_matrix_tree$byClass["Sensitivity"], "\n")
...
plot(tree_model$finalModel, uniform = TRUE, 
     main = "Decision Tree", 
     sub = paste("Optimal Tree Depth:", tree_model$bestTune$cp))
text(tree_model$finalModel, use.n = TRUE, cex = 0.6)
